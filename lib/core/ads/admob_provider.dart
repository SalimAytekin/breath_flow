...
class AdMobProvider implements AdProvider {
  ...
  String? _currentInterstitialPlacement;
  String? _currentRewardedPlacement;
  ...
  Future<void> loadBanner({required String placement, AdSize? size}) async {
    try {
      ...
      // ⚡ OPTIMIZE: Sadece kritik loglar
      if (kDebugMode) {
        print('🎯 Banner yükleniyor - Placement: $placement');
      }
      
      // Daha agresif AdRequest konfigürasyonu
  ...
  Future<void> loadInterstitial({required String placement}) async {
    try {
      ...
      if (kDebugMode) {
        ...
        print('📱 Ad Unit ID: ${AdConfig.interstitialAdUnitId}');
      }
