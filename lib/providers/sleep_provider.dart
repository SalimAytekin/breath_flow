...
class SleepProvider extends ChangeNotifier {
  ...
  /// Aralık bazlı algoritma:
  /// - < 5 saat: 0-20 puan (çok yetersiz)
  /// - 5-7 saat: 20-80 puan (yetersiz ama artıyor)
  /// - 7-9 saat: 80-100 puan (sağlıklı aralık, 8 saatte 100)
  /// - 9+ saat: 80'den azalır (fazla uyku)
  int get sleepQualityScore {
    try {
      ...
      for (var entry in entriesWithData) {
        totalScore += sleepQualityScoreForEntry(entry);
      }

      // Ortalamayı al ve tam sayıya yuvarla
      return (totalScore / entriesWithData.length).round().clamp(0, 100);
    } catch (e, stackTrace) {
  ...
  /// Belirli bir uyku kaydı için kalite skorunu hesaplar (0-100).
  int sleepQualityScoreForEntry(SleepEntry entry) {
    final actualHours = entry.actualSleep.inMinutes / 60.0;
    final targetHours = entry.targetSleep.inMinutes / 60.0;
    return _calculateQualityScore(actualHours, targetHours);
  }

  int _calculateQualityScore(double actualHours, double targetHours) {
    if (actualHours <= 0) return 0;

    double score;

    if (actualHours < 5.0) {
      score = (actualHours / 5.0 * 20).clamp(0, 20);
    } else if (actualHours >= 7.0 && actualHours <= 9.0) {
      final distanceFromTarget = (targetHours.clamp(7.0, 9.0) - actualHours).abs();
      score = (100.0 - (distanceFromTarget * 15.0)).clamp(80, 100);
    } else if (actualHours < 7.0) {
      score = (20.0 + ((actualHours - 5.0) / 2.0 * 60.0)).clamp(20, 80);
    } else {
      final excess = actualHours - 9.0;
      score = (80.0 - (excess * 15.0)).clamp(20, 80);
    }
  ...
  String formatDuration(Duration duration) {
    ...
    return '${minutes}${AppStrings.minuteShortSuffix}';
  }
