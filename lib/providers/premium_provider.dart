...
class PremiumProvider extends ChangeNotifier {
  ...
  bool get isPremiumUser {
    // 🔧 Debug modda tüm içeriklere erişim aç - İPTAL EDİLDİ (Reklam testi için)
  ...
  static const int freeMixLimit = 2;
  ...
  void _setupPaymentServiceCallback() {
    ...
    // 🔔 Premium süresi dolduğunda tetiklenecek callback
    _paymentService.onPremiumExpired = () async {
      if (kDebugMode) debugPrint('🔔 PremiumProvider: Premium süresi doldu bildirimi alındı');
  ...
  Future<bool> _restoreWithRetry({int maxRetries = 3}) async {
    final result = await _paymentService.restoreWithRetry(maxRetries: maxRetries);
    return result ?? false;
  }
