...
class BreathingProvider extends ChangeNotifier {
  ...
  void start() {
    ...
    _preCountdown = 3;
    _completedCycles = 0;
    _stepIndex = -1; // Start with -1 since we are counting down
    _currentStep = null;
    _stepProgress = 0.0;
  ...
  void _nextStep() {
    ...
    if (_stepIndex >= _currentExercise!.steps.length) {
      ...
      if (_completedCycles >= _totalCycles) {
        ...
        // _actualSessionDurationSeconds zaten tick bazlı, ek hesaplama gerekmez
  ...
  void _tick(Timer timer) {
    ...
    // Tick bazlı süre sayacı — pause'da timer durduğu için otomatik durur
    _actualSessionDurationSeconds++;
    
    final int stepDuration = _currentStep!.duration;
    // Calculate progress (0.0 to 1.0)
    if (stepDuration > 0) {
        _stepProgress = (_countdown - 1) / stepDuration;
    } else {
        _stepProgress = 1.0;
    }
    
    if (_countdown > stepDuration) {
      _nextStep();
    }
    notifyListeners();
  }
