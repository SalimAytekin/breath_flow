...
class EntryChoiceActivity : AppCompatActivity(), ActivityCompat.OnRequestPermissionsResultCallback {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // Java butonu görünür tutuyoruz
    findViewById<TextView>(R.id.java_entry_point).setOnClickListener {
      startLivePreviewActivity()
    }
    
    // Kotlin butonu kaldırıldı
    
    // İzinleri kontrol et
    if (!allRuntimePermissionsGranted()) {
      getRuntimePermissions()
    } else {
      // İzinler zaten varsa hemen LivePreviewActivity'yi başlat
      startLivePreviewActivity()
    }
  }
  ...
  private fun startLivePreviewActivity() {
    ...
    // Ana ekrana dönüşü engellemek için bu aktiviteyi sonlandır
    finish()
  }
  ...
  override fun onRequestPermissionsResult(
    ...
    if (requestCode == PERMISSION_REQUESTS) {
      if (allRuntimePermissionsGranted()) {
        // İzinler verilince LivePreviewActivity'yi başlat
        startLivePreviewActivity()
      } else {
        Log.e(TAG, "Fizik tedavi uygulaması için kamera izni gerekiyor")
      }
    }
  }
